let puzzleInput = `Game 1: 5 red, 1 green; 6 red, 3 blue; 9 red; 1 blue, 1 green, 4 red; 1 green, 2 blue; 2 blue, 1 red
...
let regexGreen = /\d+ green/g;
let regexRed = /\d+ red/g;
...
function workingGames() {
    ...
    for (x in puzzleArray) {
        ...
        while (y < temp.length && gameOn) {
            if(parseInt(temp[y].replace(/\D/g, '')) > maxGreen){
                gameOn = false;
            }
            y++;
        }
        ...
        while (y < temp.length && gameOn) {
            if(parseInt(temp[y].replace(/\D/g, '')) > maxRed){
                gameOn = false;
            }
            y++;
        }
        ...
        while (y < temp.length && gameOn) {
            if(parseInt(temp[y].replace(/\D/g, '')) > maxBlue){
                gameOn = false;
            }
            y++;
        }
        if(gameOn){
            temp = puzzleArray[x].match(regexGame);
            sumOfGames += parseInt(temp[0].replace(/\D/g, ''));
        }
    }
